/* ============================================================
   Aronui — motif library
   Three NZ-grounded visual systems, translated into SaaS language:
   · pathways — knowledge pathways, connected learning nodes
   · woven    — modern woven knowledge (kete/whatu, abstracted)
   · journey  — learning journeys, wayfinding & navigation
   All exported to window. Every motif is deterministic SVG —
   no photos, no koru clipart, no tourism imagery.
   ============================================================ */

const M_TEAL = '#0e7c72', M_NAVY = '#16263d', M_GOLD = '#c8922a';

/* tiny 4-point navigation star (celestial wayfinding, abstracted) */
function NavStar({ x = 0, y = 0, s = 6, color = M_GOLD, opacity = 1 }) {
  return (
    <path transform={`translate(${x} ${y}) scale(${s / 7})`} opacity={opacity} fill={color}
      d="M0 -7 L1.7 -1.7 L7 0 L1.7 1.7 L0 7 L-1.7 1.7 L-7 0 L-1.7 -1.7 Z" />
  );
}

/* ============ HERO BACKDROPS (1440×560, fade handled by CSS mask) ============ */
...
function HeroPathways() {
  ...
  return (
    <svg className="motif-hero-svg" viewBox="0 0 1440 560" preserveAspectRatio="none" aria-hidden="true">
      ...
      {/* a branch — knowledge diverging then reconnecting */}
...
function HeroWoven() {
  /* loose horizontal strands crossed by verticals that break where they
     pass under — whatu/raranga structure, abstracted to a lattice */
  ...
  vx.forEach((x, xi) => {
    let prev = 30;
    hy.forEach((y, yi) => {
      const under = (xi + yi) % 2 === 0; // alternate over/under
  ...
  return (
    <svg className="motif-hero-svg" viewBox="0 0 1440 560" preserveAspectRatio="none" aria-hidden="true">
      ...
      {vx.map((x, i) => <circle key={'d' + i} cx={x} cy={i % 2 ? 405 : 215} r="3.4" fill={i % 3 === 2 ? M_GOLD : M_TEAL} opacity=".75" />)}
    </svg>
  );
}

function HeroJourney() {
  const miles = [[260, 392], [640, 318], [1020, 222]];
  return (
    <svg className="motif-hero-svg" viewBox="0 0 1440 560" preserveAspectRatio="none" aria-hidden="true">
      {/* faint compass arcs, corner */}
      {[160, 240, 320].map(r => (
      ...
      {/* navigation stars */}
      <NavStar x={520} y={120} s={5} opacity=".5" />
      <NavStar x={860} y={88} s={7} opacity=".65" />
      <NavStar x={1150} y={140} s={4.5} opacity=".45" />
      <NavStar x={300} y={200} s={4} color={M_TEAL} opacity=".4" />
      <NavStar x={1330} y={60} s={5.5} opacity=".55" />
    </svg>
  );
}

function MotifHero({ dir }) {
  return (
...
/* ============ SECTION DIVIDERS (inline, centred) ============ */

function MotifDivider({ dir }) {
  return (
...
function MotifMark({ dir, className = 'logo-mark' }) {
  return (
...
function MotifLogo({ dir, light = false }) {
  return (
    <a href="index.html" className={'logo' + (light ? ' logo-light' : '')}>
      <MotifMark dir={dir} />
...
/* ============ EYEBROW TICK (replaces flat weave bar) ============ */

function MotifTick({ dir }) {
  return (
    <svg width="30" height="12" viewBox="0 0 30 12" aria-hidden="true" style={{ flex: 'none' }}>
      ...
      {dir === 'woven' && (<g stroke="currentColor">
...
/* ============ PLANNER EMPTY-STATE ART ============ */

function MotifEmpty({ dir }) {
  return (
    <svg width="150" height="64" viewBox="0 0 150 64" aria-hidden="true" style={{ marginBottom: 4 }}>
      {dir === 'pathways' && (<g>
        ...
        <circle cx="34" cy="40.5" r="3.4" fill={M_TEAL} /><circle cx="80" cy="20" r="3.4" fill={M_NAVY} opacity=".7" /><circle cx="122" cy="22" r="3.4" fill={M_GOLD} />
        <circle cx="80" cy="20" r="9" fill="none" stroke={M_NAVY} strokeWidth="1" opacity=".25" />
      </g>)}
      {dir === 'woven' && (<g>
        {[18, 32, 46].map((y, yi) => <line key={y} x1="20" y1={y} x2="130" y2={y} stroke={yi === 1 ? M_GOLD : M_TEAL} strokeWidth="1.6" opacity={yi === 1 ? .55 : .45} />)}
        {[45, 75, 105].map((x, i) => (<g key={x}>
      ...
      {dir === 'journey' && (<g>
        ...
        <NavStar x={50} y={14} s={4} opacity=".5" />
      </g>)}
    </svg>
  );
}

/* ============ DARK-SECTION BACKDROP (white strokes, very low opacity) ============ */

function MotifDark({ dir }) {
  return (
    <svg className="motif-dark-svg" viewBox="0 0 1440 700" preserveAspectRatio="none" aria-hidden="true">
      {dir === 'pathways' && (<g stroke="#fff" fill="none">
      ...
      {dir === 'woven' && (<g stroke="#fff">
        {[160, 320, 480, 640].map((y, yi) => <line key={y} x1="-20" y1={y} x2="1460" y2={y - 30} strokeWidth="1.2" opacity=".11" />)}
        {[260, 620, 980, 1280].map((x, i) => (<g key={x}>
      ...
      {dir === 'journey' && (<g>
        ...
        <NavStar x={1130} y={170} s={7} color="#fff" opacity=".24" />
        <NavStar x={780} y={120} s={5} color="#fff" opacity=".18" />
        <NavStar x={420} y={220} s={4} color="#fff" opacity=".15" />
...
Object.assign(window, { MotifHero, MotifDivider, MotifMark, MotifLogo, MotifTick, MotifEmpty, MotifDark, NavStar });
