...
function PlannerGenerator({ compact = false, autoStart = false }) {
  ...
  useEffect(() => {
    if (!autoStart) return;
    const t1 = setTimeout(() => setBusy(true), 500);
    const t2 = setTimeout(() => { setBusy(false); setDone(true); setOpen(0); }, 1800);
        ...
        <div className="planner-out">
          {!done && !busy && (
            <div className="planner-empty">
              <Icon name="doc" size={26} className="planner-empty-ic" />
...
function ResourceCard({ kind, title, meta, tone, downloads }) {
  return (
    <div className="card card-hover res-card">
      ...
      <div className="res-meta">
        ...
        {downloads && <div className="res-downloads"><Icon name="arrowup" size={12} />{downloads} downloads</div>}
      </div>
    </div>
  );
}

/* ---------------- Ask Aronui (AI chat demo) ---------------- */
const ASK_RESPONSE = [
  { icon: 'route', tone: 'teal', label: 'Curriculum Links', text: 'Language Studies › Crafting Texts › Persuasive texts (Year 9). Aligned to Knowledge on pathos, logos and ethos, and the Practice of anticipating and responding to opposing positions.' },
  { icon: 'target', tone: 'navy', label: 'Learning Intentions', text: 'We are learning to craft a persuasive text that convinces a specific audience to adopt a viewpoint or take action, making deliberate choices about structure, language and tone.' },
  { icon: 'checkc', tone: 'teal', label: 'Success Criteria', text: 'I can state a clear thesis · I can use emotive language and rhetorical questions · I can anticipate and respond to a counter-argument · I can finish with a call to action.' },
  { icon: 'layers', tone: 'navy', label: 'Lesson Activities — 45 min', text: '1 · Deconstruct a model speech for pathos/logos/ethos. 2 · Map audience and purpose. 3 · Draft thesis + two arguments. 4 · Peer-review against the success criteria.' },
  { icon: 'rubric', tone: 'gold', label: 'Assessment Ideas', text: 'Formative: annotated paragraph identifying persuasive devices. Summative: 400–500 word text for a real audience, marked against a curriculum-linked rubric.' },
  { icon: 'people', tone: 'teal', label: 'Differentiation', text: 'Scaffolded sentence stems and a model paragraph for emerging writers · Extension prompts on register and rhetorical nuance · ELL vocabulary bank with glossed terms.' },
];

function AskAronui() {
  return (
    <div className="win ask-win">
      <div className="win-bar">
        <span className="win-dots"><i style={{ background: '#e5707a' }} /><i style={{ background: '#e6b450' }} /><i style={{ background: '#5bb98c' }} /></span>
        <span className="win-title"><Icon name="spark" size={14} /> Ask Aronui</span>
        <span className="ask-model-tag"><span className="sdot teal" />Te Mātaiaho 2025</span>
      </div>
      <div className="ask-body">
        <div className="ask-prompt-row">
          <span className="ask-avatar"><Icon name="spark" size={15} /></span>
          <div className="ask-bubble">"Create a Year 9 persuasive writing lesson aligned to the new curriculum."</div>
        </div>
        <div className="ask-divider"><span className="ask-divider-label"><Icon name="spark" size={12} /> Aronui</span></div>
        <div className="ask-response">
          {ASK_RESPONSE.map((r, i) => (
            <div className="ask-item pop" key={r.label} style={{ animationDelay: (i * 60) + 'ms' }}>
              <span className={'ask-item-ic tone-' + r.tone}><Icon name={r.icon} size={15} /></span>
              <div>
                <div className="ask-item-label">{r.label}</div>
                <div className="ask-item-text">{r.text}</div>
              </div>
            </div>
          ))}
          <div className="ask-footer">
            <span className="ask-verified"><Icon name="checkc" size={14} /> Aligned to Te Mātaiaho · Language Studies · Year 9</span>
            <a href="#" className="btn btn-teal btn-sm">View full lesson plan <Icon name="arrow" size={14} /></a>
          </div>
        </div>
      </div>
    </div>
...
Object.assign(window, { CURRICULUM, PlannerGenerator, CurriculumBrowser, ToolCard, StepCard, ResourceCard, PLANNER_OUTPUT, AskAronui });
